<h1 align="center">Chicago-Crime-Sim</h1>
<p align="center">
  <strong>GIS Spatial Monitor & HCI-Driven Urban Safety Simulation Pipeline</strong>
</p>
<p align="center">
  <!-- 如果有项目图片，可以在这里居中放置 -->
  <img src="HI-FI.png" width="80%" />
</p>
...
graph LR
    %% 样式与配色定义 (学术线稿风)
    classDef boxStyle fill:#ffffff,stroke:#1a1a1a,stroke-width:2px,color:#1a1a1a,font-weight:bold;
    classDef subBoxStyle fill:#f8f9fa,stroke:#d1d5db,stroke-width:1px,stroke-dasharray: 5 5,color:#4b5563;
    classDef highlightBox fill:#f0f7ff,stroke:#2563eb,stroke-width:2px,color:#1e40af,font-weight:bold;

    %% 核心数据层
    A["城市空间大数据<br>(GIS / POI / Weibo)"]:::boxStyle

    %% 后端计算子图
    subgraph Backend [" 后端计算与控制 (Backend Simulation) "]
        B["AI / 机器学习预测模型"]:::highlightBox
        D["规划政策解析器<br>(PDF / ADU Policy)"]:::boxStyle
        F["空间参数干预矩阵<br>(FAR / Height / Lights)"]:::boxStyle
    ...
    %% 前端交互子图
    subgraph Frontend [" 前端交互与感知 (HCI Interface) "]
        C["移动端交互界面<br>(Mobile App UX)"]:::highlightBox
        E1["24H 动态安全评估报告<br>(Safety Report)"]:::boxStyle
        E2["实时危险分布网格<br>(Spatial Monitor)"]:::boxStyle
    ...
    %% 子图样式应用
    style Backend fill:#fafafa,stroke:#e5e7eb,stroke-width:1px
    style Frontend fill:#fafafa,stroke:#e5e7eb,stroke-width:1px
```
